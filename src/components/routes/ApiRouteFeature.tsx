...
const GetApiResource = (props: IContainerProps) => {
    ...
    const reloadApis = async () => {
        try {
            const response = await apiService.getApi(getApiId());
...
const View = (props: IViewProps) => {
    return (
        <Container>
            <Grid>
                <Grid.Row>
                    <Button labelPosition='left' icon='left chevron' size='mini' content={`API: ${props.selectedApi.name} | PORT: ${props.selectedApi.port}`}
